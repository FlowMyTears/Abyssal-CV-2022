...
public:
    ...
    static inline float GetTwoPiontDistance(cv::Point2f point_1, cv::Point2f point_2) {
    ...
    static void getShortCenter(cv::RotatedRect rect, cv::Point2f* shortCenter) {
        ...
        cv::Point2f verts[4];
        rect.points(verts);
        if (GetTwoPiontDistance(verts[0], verts[1]) < GetTwoPiontDistance(verts[1], verts[2])) {
    ...
    static cv::RotatedRect assembleArmor(cv::RotatedRect rect0, cv::RotatedRect rect1){
        ...
        float width = GetTwoPiontDistance(pl, pr);
    ...
    static bool embeddedRectJudge(cv::RotatedRect r1, cv::RotatedRect r2) {
        ...
        return (GetTwoPiontDistance(r1.center, r2.center) < distance);
    }
